...
def test_optimisation_pipeline_is_deterministic(tmp_path: Path) -> None:
    first = _run(tmp_path / "a.db", tmp_path / "a")
    second = _run(tmp_path / "b.db", tmp_path / "b")

    assert first["optimisation_run_id"] == second["optimisation_run_id"]
...
def _run(database: Path, output: Path) -> dict[str, object]:
    ...
    return run_optimisation(database_path=database, output_dir=output, rebuild=True)
