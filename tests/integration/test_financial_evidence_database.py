...
def test_financial_evidence_tables_are_populated(tmp_path: Path) -> None:
    ...
    run_financial_analysis(database_path=database, output_dir=None, rebuild=True)

    connection = sqlite3.connect(database)
    try:
        assert connection.execute("SELECT COUNT(*) FROM evidence_financial_runs").fetchone()[0] == 1
        assert (
            connection.execute("SELECT COUNT(*) FROM evidence_financial_case_catalogue").fetchone()[
                0
            ]
            == 7
        )
        assert (
            connection.execute("SELECT COUNT(*) FROM evidence_financial_cashflows").fetchone()[0]
            == 105
        )
        assert (
            connection.execute(
                "SELECT COUNT(*) FROM evidence_financial_confidence "
                "WHERE readiness_status = 'not_realisable_without_mitigation'"
            ).fetchone()[0]
            == 7
        )
    finally:
