...
def test_run_financial_analysis_cli(tmp_path: Path) -> None:
    database = tmp_path / "financial-cli.db"
    ...
    result = CliRunner().invoke(
        ...
        [
            ...
            str(database),
            "--output-dir",
            str(tmp_path / "financial"),
            "--rebuild",
        ],
    )

    assert result.exit_code == 0
    assert "Financial analysis run complete:" in result.stdout
    assert (tmp_path / "financial" / "financial_run_summary.json").is_file()
